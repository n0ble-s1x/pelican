...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let target = root.iter().find(|o| o.is_folder() && o.filename.eq_ignore_ascii_case("Audiobooks")).ok_or("no audiobooks")?;
    let bytes = std::fs::read("/tmp/probe.mp3")?;
    let info = NewObjectInfo::file("probe.mp3", bytes.len() as u64);
    let stream = futures::stream::iter(bytes.chunks(256*1024).map(|c| Ok::<_, std::io::Error>(Bytes::copy_from_slice(c))).collect::<Vec<_>>());
    let h = storage.upload(Some(target.handle), info, Box::pin(stream)).await?;
    eprintln!("uploaded handle: {:?}", h);

    // re-fetch storages
    let storages2 = device.storages().await?;
    let after = storages2[0].info().free_space_bytes;
    eprintln!("free after: {}, delta: {}", after, before as i64 - after as i64);

    // List /Audiobooks contents
    let session = device.session();
    let storage_id = storage.id();
    let handles = session.get_object_handles(storage_id, None, Some(target.handle)).await?;
